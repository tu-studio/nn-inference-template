...
void warmUpInference(torch::jit::Module& module,
...
[[clang::realtime]] void runInference(torch::jit::Module& module,
                                      const std::vector<torch::jit::IValue>& inputs,
                                      at::Tensor& outputTensor)
{
    outputTensor = module.forward(inputs).toTensor();
...
int main(int argc, const char* argv[]) {
    ...
    warmUpInference(module, inputs, outputTensor);

    std::cout << "Warm-Up finished" << std::endl;

    runInference(module, inputs, outputTensor);
