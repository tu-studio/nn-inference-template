...
#include "dsp/inference/InferenceConfigNew.h"
...
#ifdef USE_LIBTORCH
        GUITARLSTM_MODELS_PATH_TENSORFLOW + std::string("/") +std::string("model_0/model_0-streaming.tflite"),
...
InferenceConfig config(
#ifdef USE_LIBTORCH
        STEERABLENAFX_MODELS_PATH_PYTORCH + std::string("/") + std::string("model_0/steerable-nafx-2048.pt"),
        {BATCH_SIZE, 1, MODEL_INPUT_SIZE_BACKEND},
...
InferenceConfig config(
#ifdef USE_LIBTORCH
        STATEFULLSTM_MODELS_PATH_PYTORCH + std::string("/") + std::string("model_0/stateful-lstm.pt"),
